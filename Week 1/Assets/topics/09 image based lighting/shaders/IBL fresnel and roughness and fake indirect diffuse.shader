Shader "examples/week 9/IBL fresnel and roughness and fake indirect diffuse"
{
    Properties 
    {
        ...
        _reflectionBoost ("Reflection Boost", Range(0, 2)) = 1
        _diffuseBoost ("Diffuse Boost", Range(0, 2)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (Interpolators i) : SV_Target
            {
                ...
                float adj = 0.5;
                adj = max(0,dot(viewDirection, lightDirection));
               
                float3 indirectSpecularOrg = texCUBElod(_IBL, float4(viewReflection, mip));
                float3 indirectSpecularAdj = 1- indirectSpecularOrg;
                float3 indirectSpecular = lerp(indirectSpecularAdj, indirectSpecularOrg, adj);
                ...
                float3 indirectDiffuseOrg = texCUBElod(_IBL, float4(normal, DIFFUSE_MIP_LEVEL));
                float3 indirectDiffuseAdj = 1- indirectDiffuseOrg;
                float3 indirectDiffuse = lerp(indirectSpecularAdj,indirectDiffuseOrg, adj);
