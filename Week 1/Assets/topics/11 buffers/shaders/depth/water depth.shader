Shader "examples/week 11/water depth"
{
    ...
    SubShader
    {
        ...
        GrabPass{ // grab pass for background texture
            "_BackgroundTex"
            }
        ...
        Pass
        {
            ...
            float4 frag (Interpolators i) : SV_Target
            {
                ...
                float2 distoredScreenUV = screenUV + float2(0.1,0.4) * (w * i.normal.y) * _refractionIntensity; //distortion

                float3 background = tex2D(_BackgroundTex, distoredScreenUV).rgb;

                float distortedDepth = Linear01Depth(tex2D(_CameraDepthTexture, distoredScreenUV)).r;
                float distortedDepthDiff = abs((distortedDepth/_ProjectionParams.w) - i.surfZ);
                float underwaterDepth = 1-smoothstep (0, _depthFog, distortedDepthDiff);

                background *= underwaterDepth;

                color = saturate(color + background * (1-_opacity));//_opacity * background + (1-_opacity) * color;
                //color = lerp(color, background, _opacity);
                //color = color + _opacity * background;
                color += smoothstep(0.1,0.2,intersection) * 0.5; // add interaction
                color += pow(distoredScreenUV.y,5) * float3(1,0,0) * 0.8; // add some horizontal gradient
